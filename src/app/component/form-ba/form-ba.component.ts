...
@Component({
  ...
  styleUrls: ['./form-ba.component.scss']
})
export class FormBAComponent implements OnInit {
  ...
  fetchDataAdminBA() {
    axios.get(`${environment.apiUrl2}/admin/ba/all`,
      {
        headers: {
  ...
  FetchDataUserBA() {
    axios.get(`${environment.apiUrl2}/api/my/form/ba`,
      {
        headers: {
          Authorization: `Bearer ${this.cookieService.get('userToken')}`
        }
  ...
  addBA() {
    ...
    console.log('Document UUID:', this.document_uuid);
    axios.post(`${environment.apiUrl2}/api/add/ba`, data,
      {
        headers: {
          Authorization: `Bearer ${this.cookieService.get('userToken')}`
        }
  ...
  getSpecBAITCM(form_uuid: string) {
    axios.get(`${environment.apiUrl2}/form/ba/${form_uuid}`)
    .then(response => {
      console.log(response.data);
  ...
  updateBA() {
    axios.put(`${environment.apiUrl2}/api/form/ba/update/${this.form_uuid}`, {
      formData: {
        document_uuid: this.document_uuid,
        form_ticket: this.form_ticket,
        project_uuid: this.project_uuid,
