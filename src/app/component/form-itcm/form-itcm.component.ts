...
import { FormItcmService } from '../component-service/form-itcm-service/form-itcm-service.service';
...
export class FormITCMComponent implements OnInit {
  ...
  fetchDataAdminFormITCM() {
    axios.get(`${environment.apiUrl2}/admin/itcm/all`, {
      headers: {
  ...
  fetchDataUserFormITCM() {
    axios.get(`${environment.apiUrl2}/api/my/form/itcm`,
      {
        headers: {
          Authorization: `Bearer ${this.cookieService.get('userToken')}`
        }
  ...
  getSpecificFormITCM(form_uuid: string) {
    axios.get(`${environment.apiUrl2}/form/itcm/${form_uuid}`)
      .then((response) => {
        console.log(response.data);
        $('#updateModalITCM').modal('show');
  ...
  getSpecificFormITCMDetail(form_uuid: string) {
    axios.get(`${environment.apiUrl2}/itcm/${form_uuid}`)
      .then((response) => {
        $('#detailModalITCM').modal('show');
